...
# (Configurado con ZRAM, Pacman optimizado, Microcodigo AMD, Mesa Vulkan/RADV/VA-API, PipeWire)
...
if command -v wine &>/dev/null || command -v proton &>/dev/null; then
    ...
    sudo pacman -S --needed --noconfirm lib32-vulkan-radeon lib32-mesa lib32-libva-mesa-driver 2>/dev/null || true
fi
