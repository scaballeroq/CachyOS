#!/bin/bash
# post-install.sh - Despachador y selector inteligente de post-instalacion para CachyOS (Arch Linux + GNOME)
...
show_help() {
    cat <<EOF
Despachador de Post-Instalacion para CachyOS (Arch Linux + GNOME)
...
Opciones:
  ...
  --intel, -i        Ejecuta la configuracion optimizada para Intel Core + Intel Graphics.
  --help, -h         Muestra este mensaje de ayuda.

Scripts independientes disponibles:
  - Setup/post-install-amd.sh   -> Optimizado para AMD Ryzen (firmware AMD, RADV, Mesa, Chaotic-AUR, GNOME, Pacman)
  - Setup/post-install-intel.sh -> Optimizado para Intel Core (microcodigo Intel, VA-API Intel, Chaotic-AUR, GNOME, Pacman)
EOF
}
...
if [ $# -gt 0 ]; then
    case "$1" in
        --amd|-a|amd)
            ...
            ;;
        --intel|-i|intel)
            echo "Opcion manual seleccionada: Intel Core"
