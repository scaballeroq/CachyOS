...
Environment management is centralized through **Mise** (runtimes and SDKs) and **Rustup** (Rust toolchain), supplemented by automated tasks configured via a `justfile` and natively integrated with **GNOME (Wayland / systemd user session)** as well as **Zsh** and **Bash** shells.
...
* **Installation**: Installs the latest Long Term Support **LTS** version of the .NET SDK:
  ...
  ```
* **GNOME & IDEs**: Configures `DOTNET_ROOT` in `~/.config/environment.d/10-dotnet.conf` for JetBrains Rider, VS Code, and Antigravity, while opting out of build telemetry.
...
* **GNOME Integration**: Exports `JAVA_HOME=/usr/lib/jvm/default` in `~/.config/environment.d/10-java.conf` for Android Studio, IntelliJ IDEA, Gradle, and Maven.
