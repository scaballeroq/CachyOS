...
# Optimizado para CachyOS / Arch Linux (Zero interferencia con el sistema)
# ==============================================================================
...
if ! command -v uv &>/dev/null; then
    ...
    echo "Instálalo vía Mise ('mise use -g uv@latest') o con pacman ('sudo pacman -S uv')."
    exit 1
fi
...
app = FastAPI(
    ...
    description="Proyecto Python aislado creado con uv en CachyOS",
    version="1.0.0"
)
...
def saludo(nombre: str = "Usuario"):
    ...
    console.print(Panel.fit(
        ...
        title="[bold yellow]CachyOS Python CLI[/bold yellow]"
    ))
